...
(function () {
  ...
  function showQuestion() {
    ...
    els.questionText.innerHTML = formatRichText(q.question);
    ...
    order.forEach((key) => {
      ...
      btn.innerHTML = `
        ...
        <span class="answer-text">${formatRichText(q.options[key])}</span>
      `;
  ...
  /**
   * Render question/explanation HTML with LTR formula spans.
   * Backtick-wrapped segments (e.g. `(∀x)(∀y)(Axy → Ayx)`) stay left-to-right
   * inside RTL Hebrew text — matching the logical order of PL notation.
   */
  function formatRichText(str) {
    const escaped = escapeHtml(str);
    // `formula` → <span class="formula-ltr" dir="ltr">formula</span>
    return escaped.replace(/`([^`]+)`/g, '<span class="formula-ltr" dir="ltr">$1</span>');
  }
  ...
  function showExplanation(q) {
    ...
    els.correctBanner.innerHTML = `
      ...
      <div>
        ...
        <div style="opacity:0.9;font-size:1.02rem;">${formatRichText(correctText)}</div>
      </div>
    `;
    els.explainText.innerHTML = formatRichText(display.explanation);
  ...
  function buildStudyPlan(weakTopics) {
    const tips = {
      ...
      // לוגיקה ב — פול פאוור (מבחן חזרה 2 · 50 שאלות)
      "תמסיר 1 · מונחים, פרדיקטים ו-SL": [
        "חזרו על טיעון דן/נועה: למה סימול ב-SL מאבד תוקף.",
        "שם פרטי = מוסכמה; תיאור מיידע = מתאר יחיד; לא כל מונח מציין.",
        "פתחו תמסיר 1 שורות 25–59 וסמנו את ההבחנות.",
      ],
      "תמסיר 2 · תחביר PL": [
        "אוצר סימנים: אותיות, פרדיקטים, קבועים/משתנים, קשרים, ∀∃.",
        "נוסחה אטומית: אות פסוקית או An t1…tn — בלי אופרטור ראשי.",
        "זהו אופרטור ראשי: ב-∼(Rabz & Hxy) הראשי הוא ∼, לא &.",
      ],
      "תמסיר 3 · כמתים ומקרי הצבה": [
        "פסוק = אין מופע חופשי; Rabz אינה פסוק (z חופשי).",
        "מקרה הצבה: רק השמטת הכמת הפותח + הצבת קבוע.",
        "תרגלו: Fab, Fbb, Fcb כמקרי הצבה של (∀z)Fzb.",
      ],
      "תמסיר 4 · הצרנות בסיסיות": [
        "(∃y)Lym & (∃y)∼Lym ≠ (∃y)(Lym & ∼Lym) — שני אנשים מול סתירה.",
        "מישהו…וגם… = (∃x)(P & Q); אם מישהו… = (∃x)P → Q או (∀x)(P → Q).",
        "זהרו מ-(∀x)Rxr → Rhr כשמחפשים 'אם מישהו…'.",
      ],
      "תמסיר 5 · הצרנות מתקדמות": [
        "כל…לפחות אחד = (∀x)(∃y)Lxy; יש…שכל = (∃z)(∀w)Lzw — לא להחליף!",
        "אף…אינו… = ∼(∃x)… או (∀x)∼…",
        "נמרים וזברות: פיצול ∀ מול איחוד עם ∨ ברישא.",
      ],
      "תמסיר 6 · ריבוע הניגודים וכמתים מקוננים": [
        "E ≡ ∼I: (∀x)(P → ∼Q) ≡ ∼(∃x)(P & Q).",
        "יש קטן ביותר: (∃y)∼(∃x)Lxy — יחידות דורשת זהות.",
        "כוונות / חיפוש: לא תמיד (∃x)(Ox & Lmx) — לפעמים פסוק אטומי.",
      ],
      "תמסיר 7 · סדר כמתים ושקילויות": [
        "בחירת פרדיקטים לטיעון: אל תאחדו מידע שצריך לגשר (עטלפים).",
        "כמתים מאותו סוג: (∀x)(∀y)… ≡ (∀y)(∀x)…",
        "סדר ∀∃ מול ∃∀ משנה משמעות — תרגלו על Lxy.",
      ],
      "תמסיר 8 · זהות וכמויות": [
        "כל שאר…: (∀y)[∼(y = h) → …] תחת תחום מתאים.",
        "בדיוק שניים = לפחות שניים + אין שלישי (יחידות עם =).",
        "לכל היותר n לא טוען שיש בכלל — רק שאין יותר מ-n.",
      ],
      "תמסיר 9 · יחסים ופונקציות": [
        "זהות = טרנזיטיבי + סימטרי + רפלקסיבי; 'חבר של' אינו טרנזיטיבי.",
        "פונקציה: ערך יחיד + הערך בתחום הדיון.",
        "מונח מורכב עם משתנה = נוסחה/פסוק? בדקו קשירה.",
      ],
      "תמסיר 10 · סמנטיקה ופירושים": [
        "מונח סגור = בלי משתנים; פירוש מתאים ערכי-אמת, קבועים, אקסטנציות.",
        "n-יה היא סדורה: ⟨1,2,3⟩ ≠ ⟨3,2,1⟩.",
        "פתחו תמסיר 10 שורות 86–114.",
      ],
      "תמסיר 11 · השמות וסיפוק": [
        "פסוק פתוח צריך השמה, לא רק פירוש.",
        "תיקון d[u/x] משנה רק את x; סיפוק ∀/∃ דרך תיקונים.",
        "תרגלו על פירוש 1: (∃x)(Ex & Pxxx) שקרי למרות E={2,4}.",
      ],
      "תמסיר 11–12 · סיפוק ופירושים": [
        "קוניונקציה: בודקים כל קוניונקט בנפרד — לא חייבים אותו תיקון.",
        "קבוע: den נקבע מהפירוש, לא מההשמה.",
        "תרגלו על פירוש 3 + d₂: Lxy, Lyz, Tzxb, (∀x)(Px→∼Ex), (∃x)Taax.",
      ],
